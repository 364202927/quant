...
class realTrade(baseTrade):
    ...
    # 现货订单固定逻辑:type/symbol/lv/posSide都是套路,调用方只传真正变化的值
    ...
    # ── 合约 ──
    def openLong(self, symbol: str, totelPrice: float | str, orderBook: int = 0, price: float | None = None,lv: int = 0, inForce: str = 'GTC', exName: list[str] | None = None) -> None:
        data = self._swapOrder(symbol, kBuy, kLong, totelPrice, orderBook, price, lv, inForce)
        self._dispatch(eMarketId['submit'], exName, data)

    def openShort(self, symbol: str, totelPrice: float | str,orderBook: int = 0, price: float | None = None, lv: int = 0, inForce: str = 'GTC', exName: list[str] | None = None) -> None:
        data = self._swapOrder(symbol, kSell, kShort, totelPrice, orderBook, price, lv, inForce)
        self._dispatch(eMarketId['submit'], exName, data)

    def closePos(self, symbol: str, dir: str, totelPrice: float | str = 'bet:100', orderBook: int = 0,price: float | None = None, lv: int = 0, inForce: str = 'GTC', exName: list[str] | None = None) -> None:
        data = self._swapOrder(symbol, kClose, dir, totelPrice, orderBook, price, lv, inForce)
        self._dispatch(eMarketId['submit'], exName, data)
